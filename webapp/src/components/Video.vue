<template>
    <div>
        ...
        <!--视频列表-->
        <ul id="videolist">
            <li v-for="v in lesson">
                <router-link :to="{params:{lessonId:v.id},name:'Page'}" class="pic">
                    <img :src="v.preview"/>
                    <span>08:26</span>
                    <i class="iconfont icon-bofang"></i>
                </router-link>
                <router-link :to="{params:{lessonId:v.id},name:'Page'}"  class="title">
                     {{ v.title }}
                </router-link>
            </li>
        </ul>
...
<script>
    export default {
        ...
        methods:{
            loadData(){
                //获取标签
                this.axios.get('http://www.jjjgd.com/tags').then((response) => {
                    this.tags = response.data.data;
                })
                //获取课程
                let tid = this.$route.params.tid;
                this.axios.get('http://www.jjjgd.com/lesson/' + (tid ? tid : 0)).then((response) => {
